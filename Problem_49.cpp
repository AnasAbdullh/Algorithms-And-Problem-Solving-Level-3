...
void CheckNumberAccount(vector <sClient> vCleints,string  NumberAccount)
{
    bool check = true;

    for (short i = 0 ; i < vCleints.size() ; i++) {
        if (NumberAccount == vCleints[i].AccountNumber) {
            printReadNewClient(vCleints[i]);
            check = false;
        }
    }
    if (check) {
...
int main()
{
    ...
    CheckNumberAccount(vCleints,NumberAccount);
